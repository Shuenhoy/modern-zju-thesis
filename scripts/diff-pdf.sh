...
function diff_page {
    ...
    # 2+x faster
    (cat $pdf_file1 | pdftoppm -f $page_number -singlefile -gray - | magick - miff:- ; \
     cat $pdf_file2 | pdftoppm -f $page_number -singlefile -gray - | magick - miff:- ) | \
    magick - \( -clone 0-1 -compose darken -composite \) \
            -channel RGB -combine $DIFFDIR/$page_number.jpg

    # 2x faster (breaks when using TIFF format instead of JPEG, and PNG is slow)
#    (pdftocairo -f $page_number -singlefile -jpeg $pdf_file1 -gray - | magick - miff:- ; \
#     pdftocairo -f $page_number -singlefile -jpeg $pdf_file2 -gray - | magick - miff:- ) | \
#    magick - \( -clone 0-1 -compose darken -composite \) \
#            -channel RGB -combine $DIFFDIR/$page_number.jpg
...
#            -highlight-color blue $DIFFDIR/$page_number.png
    ...
    if (($? > 0)); then
        echo "Problem running pdftoppm or magick!"
        exit 1
    fi
    grayscale=$(magick $DIFFDIR/$page_number.jpg -colorspace HSL -channel g -separate +channel -format "%[fx:mean]" info:)
    if [ "$grayscale" != "0" ]; then
